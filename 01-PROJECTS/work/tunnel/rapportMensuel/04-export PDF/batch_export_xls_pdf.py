...
def detect_axes_separators(ws, *, ref_color: Optional[int], row_axis: int, col_axis: int,
    ...
    for r in range(u_r1, u_r2 + 1):
        if _color_matches(_cell_color(ws, r, col_axis), ref_color, tol):
            row_seps.append(r)
            print(f"Row separator detected at row {r}")  # Log de débogage
    ...
    for c in range(u_c1, u_c2 + 1):
        if _color_matches(_cell_color(ws, row_axis, c), ref_color, tol):
            col_seps.append(c)
            print(f"Column separator detected at column {c}")  # Log de débogage

    row_seps = sorted(set(row_seps))
    col_seps = sorted(set(col_seps))
    print(f"Row separators: {row_seps}")  # Log de débogage
    print(f"Column separators: {col_seps}")  # Log de débogage
...
def build_zones_from_separators(ws, row_seps: list[int], col_seps: list[int]) -> list[str]:
    ...
    for j in range(1, len(row_seps)):
        ...
        for i in range(1, len(col_seps)):
            ...
            zone = ws.Range(top_left, bottom_right).Address
            zones.append(zone)
            print(f"Zone built: {zone}")  # Log de débogage
    return zones
...
def _is_grey_row_span(ws, r: int, c1: int, c2: int, ref_color: int | None, tol: int = 0) -> bool:
    ...
    for c in range(c1, c2 + 1):
        ...
        if clr is None or not _color_matches(clr, ref_color, tol):
            return False
    return True

def _is_grey_col_span(ws, c: int, r1: int, r2: int, ref_color: int | None, tol: int = 0) -> bool:
    if ref_color is None:
        return False
    r1 = max(1, r1)
    r2 = max(r1, r2)
...
def export_grey_zones_to_pdfs(ws, cfg: dict) -> List[Path]:
    ...
    try:
        ...
        if not zones:
            print("No zones detected")  # Log de débogage
            return []
        ...
        for addr in zones:
            ...
            if (r2 - r1 + 1) < min_r or (c2 - c1 + 1) < min_c:
                print(f"Zone {addr} is too small")  # Log de débogage
                continue
            if r2 < u_r1 or r1 > u_r2 or c2 < u_c1 or c1 > u_c2:
                print(f"Zone {addr} is outside UsedRange")  # Log de débogage
                continue
            key = (r1, c1, r2, c2)
            if key in seen:
                print(f"Zone {addr} already seen")  # Log de débogage
                continue

            borders_ok = True
            if strict:
                top_ok = _is_grey_row_span(ws, max(1, r1-1), max(1, c1-1), c2, ref_color, tol)
                bot_ok = _is_grey_row_span(ws, min(u_r2, r2+1), max(1, c1-1), c2, ref_color, tol)
                left_ok = _is_grey_col_span(ws, max(1, c1-1), max(1, r1-1), r2, ref_color, tol)
                right_ok = _is_grey_col_span(ws, min(u_c2, c2+1), max(1, r1-1), r2, ref_color, tol)
                borders_ok = (top_ok and bot_ok and left_ok and right_ok)
                if not borders_ok:
                    print(f"Zone {addr} borders are not OK")  # Log de débogage
                    continue
        ...
        if not kept:
            print("No zones kept for export")  # Log de débogage
            return []
