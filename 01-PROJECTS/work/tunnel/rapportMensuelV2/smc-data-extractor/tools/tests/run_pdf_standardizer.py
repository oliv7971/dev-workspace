...
# -*- coding: utf-8 -*-
"""
Script d'utilisation simple pour standardiser les PDF du rapport mensuel
========================================================================
...
def standardize_rapport_mensuel():
    ...
    if not os.path.exists(source_dir):
        print(f"❌ ERREUR : Dossier source introuvable :")
        print(f"   {source_dir}")
        print()
        print("💡 Solution :")
    ...
    try:
        ...
        if standardized_files:
            print(f"\n✅ {len(standardized_files)} fichier(s) standardisé(s) avec succès !")
            print("\n📊 RAPPORT DÉTAILLÉ :")
            ...
            if response.lower() in ['o', 'oui', 'y', 'yes']:
                ...
                if standardizer.merge_standardized_pdfs(standardized_files, final_path):
                    print(f"✅ Document final créé avec succès !")
                    print(f"📍 Emplacement : {final_path}")
                else:
                    print("❌ Erreur lors de la fusion")
        else:
            print("❌ Aucun fichier PDF traité avec succès")
...
def standardize_custom_directory():
    ...
    print("🔧 PDF STANDARDIZER - Dossier personnalisé")
    print("=" * 45)
    
    # Demande le dossier source
    source_dir = input("📁 Dossier contenant les PDF à traiter : ").strip().strip('"')
    ...
    # Dossier de sortie
    output_dir = os.path.join(source_dir, "PDF_standardises_A4")
    print(f"📂 Les PDF standardisés seront sauvés dans : {output_dir}")
    ...
    try:
        ...
        if standardized_files:
            print(f"\n✅ {len(standardized_files)} fichier(s) standardisé(s) !")
            ...
            if response.lower() in ['o', 'oui', 'y', 'yes']:
                ...
                if standardizer.merge_standardized_pdfs(standardized_files, final_path):
                    print(f"✅ Document fusionné : {final_path}")
        else:
...
def main():
    ...
    print("2. Traiter un dossier personnalisé")
    print("3. Quitter")
    print()
