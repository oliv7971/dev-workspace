...
class SMCExtractor:
    ...
    def extract_sheet_data(self, worksheet, sheet_name: str, file_path: Path) -> Dict[str, Any]:
        ...
        if sheet_name == 'Convergences':
            # Pour CONVERGENCES: identifier automatiquement la fin du tableau de gauche
            headers_row = 9
            data_start_row = 10
            
            # Trouver la fin du tableau de convergences en détectant les cellules fusionnées
            convergence_end_col = self.find_convergence_table_end(worksheet)
            
            # Extraction des en-têtes de convergences (tableau de gauche seulement)
            headers = {}
            excluded_headers = ['Date', 'Temps (j)', 'jours écoulés', 'Distance au front']
            ...
            for col_idx in range(1, convergence_end_col + 1):
                ...
                if header_cell:
                    header_text = str(header_cell).strip()
                    # Exclure les en-têtes non-métriques
                    if not any(excl.lower() in header_text.lower() for excl in excluded_headers):
