...
"""
Recuperation des dernieres auscultations GER.

Ce script parcourt les repertoires d'auscultation GER et recupere les 10 dernieres lignes
...
# Configuration
BASE_AUSCULTATION = Path(r"C:\data\11-CHANTIERS\BURE\22-AUSCULTATION\GER")
REPERTOIRES_GER = [
    "SMC 93",
    "SMC 109",
    "SMC 131",
...
def trouver_fichier_xlsm(repertoire: Path) -> Optional[Path]:
    ...
    fichiers_xlsm = list(repertoire.glob("*.xlsm"))
...
def main():
    parser = argparse.ArgumentParser(description="Recupere les dernieres auscultations GER depuis les fichiers Excel.")
    parser.add_argument("--output", "-o", type=Path, 
                       default=Path("dernieres_auscultations_GER.csv"),
                       help="Fichier CSV de sortie (defaut: dernieres_auscultations_GER.csv)")
    ...
    for nom_repertoire in REPERTOIRES_GER:
    ...
    if donnees_recuperees:
        ...
        print(f"\n=== RESUME ===")
        print(f"Fichiers traites avec succes : {nb_fichiers_traites}/{len(REPERTOIRES_GER)}")
