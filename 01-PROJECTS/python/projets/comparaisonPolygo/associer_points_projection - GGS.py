...
print("🚀 Début du script GGS Association")  # Message visible même sans debug

# Test pour debug - Pause forcée
import sys
if 'debugpy' in sys.modules:
    print("🐞 MODE DEBUG VS CODE DÉTECTÉ !")
    print("📍 Mettez un point d'arrêt sur cette ligne pour tester")
else:
    print("⚡ Mode exécution normale")

debug_print("Script démarré avec succès")
...
debug_print(f"Création DataFrame avec {len(résultats)} résultats")
df_resultat = pd.DataFrame(résultats)

if len(résultats) == 0:
    print("⚠️ Aucun résultat trouvé - DataFrame vide")
    # Créer un DataFrame vide avec les bonnes colonnes
    colonnes = ["Nom1", "X1", "Y1", "Z1", "Nom2", "X2", "Y2", "Z2",
                "PM1", "HZ1", "DZ1", "PM2", "HZ2", "DZ2",
                "diff_PM", "diff_HZ", "diff_DZ", "DH"]
    df_resultat = pd.DataFrame(columns=colonnes)
else:
    print(f"✅ {len(résultats)} associations trouvées")
    debug_print(f"Colonnes disponibles: {list(df_resultat.columns)}")
    colonnes = ["Nom1", "X1", "Y1", "Z1", "Nom2", "X2", "Y2", "Z2",
                "PM1", "HZ1", "DZ1", "PM2", "HZ2", "DZ2",
                "diff_PM", "diff_HZ", "diff_DZ", "DH"]
