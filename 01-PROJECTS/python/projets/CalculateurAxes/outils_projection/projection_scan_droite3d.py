...
Usage (depuis la racine du projet): 
  python outils_projection/projection_scan_droite3d.py _fichiers/scan.asc [sortie.csv] [mode] [--axe axe.txt]
...
def charger_points_axe(fichier_axe=None):
    ...
    if fichier_axe is None:
        # Axe par défaut (GGS)
        print("   Utilisation de l'axe par défaut (GGS)")
        return np.array([
            [-16.6020, 823077.4983, 1091438.1914, -122.4930],
            [0.0000, 823062.4518, 1091431.1751, -122.3270],
            [11.2790, 823052.2296, 1091426.4084, -122.2142],
            [19.0580, 823045.1794, 1091423.1208, -121.1275],
            [75.9540, 822993.6142, 1091399.0756, -120.5584],
            [85.9530, 822984.5520, 1091394.8498, -120.4584]
        ])
    else:
...
def interpoler_z_sur_axe(pm_points, points_axe):
    """
    Interpole l'altitude Z sur l'axe segment par segment.
    
    Args:
        pm_points: array (N,) - PM des points à interpoler
        points_axe: array (M, 4) - Points de l'axe [PM, X, Y, Z]
    
    Returns:
        z_interpole: array (N,) - Altitude interpolée pour chaque PM
    """
    pm_axe = points_axe[:, 0]
    z_axe = points_axe[:, 3]
    
    # Interpolation linéaire segment par segment
    z_interpole = np.interp(pm_points, pm_axe, z_axe)
    
    return z_interpole


def projeter_points_sur_droite(points_xyz, origine, direction, mode='perpendiculaire', points_axe=None):
    """
    ...
    Args:
        ...
        points_axe: array (M, 4) optionnel - Points de l'axe pour interpolation Z
    ...
    if mode == 'perpendiculaire':
        ...
        # Convention: H négatif = DROITE, H positif = GAUCHE (en regardant dans le sens croissant des PM)
        produit_vec = direction_2d[0] * residus_2d[:, 1] - direction_2d[1] * residus_2d[:, 0]
        h = np.where(produit_vec >= 0, h, -h)  # Signe standard
        
        # V = différence d'altitude par rapport à l'altitude de l'axe au PM
        if points_axe is not None:
            # Interpolation segment par segment sur l'axe réel
            z_axe = interpoler_z_sur_axe(pm, points_axe)
        else:
            # Fallback: pente moyenne linéaire
            pente_z = direction[2] / np.linalg.norm(direction[:2])
...
def traiter_fichier(fichier_entree, fichier_sortie, points_axe, mode='perpendiculaire', chunk_size=500000):
    ...
    with open(fichier_sortie, 'w') as f_out:
        ...
        for i, chunk in enumerate(pd.read_csv(fichier_entree, 
            ...
            pm, h, v = projeter_points_sur_droite(points_xyz, origine, direction, mode, points_axe)
            
            # Écrire les résultats (arrondi au millimètre)
            for j in range(len(pm)):
                f_out.write(f"{h[j]:.3f};{v[j]:.3f};{pm[j]:.3f}\n")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("="*70)
        print("\nUsage (depuis la racine du projet):")
        print("  python outils_projection/projection_scan_droite3d.py _fichiers/scan.asc [sortie.csv] [mode] [--axe axe.txt]")
        print("\nMode de projection:")
        ...
        print("  (par défaut)    - Utilise l'axe GGS codé dans le script")
        print("\nExemples:")
        print('  python outils_projection/projection_scan_droite3d.py _fichiers/scan.asc')
        print('  python outils_projection/projection_scan_droite3d.py _fichiers/scan.asc _fichiers/resultat.csv')
        print('  python outils_projection/projection_scan_droite3d.py _fichiers/scan.asc _fichiers/resultat.csv vertical')
        print('  python outils_projection/projection_scan_droite3d.py _fichiers/scan.asc --axe outils_projection/mon_axe.txt')
