...
def read_deplacements_block_strict(xl_path: Path, sheet_name: str, month_str: str) -> Tuple[List[Dict], List[Dict]]:
    """
    Lecture déterministe de l'onglet Déplacements :
    ...
    - IMPORTANT : on itère colonne par colonne (B→last_col) pour préserver l'ordre Excel.
                  Pas de dict → pas d'écrasement en cas de doublons.
    """
    ...
    # 3) balayer colonnes dans l'ordre Excel
    rows_disp: List[Dict] = []
    month_dates: set = set()

    for col_idx in range(2, last_col + 1):  # 2 = B (Date)
        header = ws.cell(row=9, column=col_idx).value
        header_str = ("" if header is None else str(header).strip())
        if not header_str or _norm(header_str) == "DATE":
            continue

        # collecter toutes les (date, valeur) de cette colonne
        pairs: List[Tuple[pd.Timestamp, Optional[float]]] = []
        ...
        rows_disp.append({
            ...
            "sheet": sheet_name,
            "month": month_str,
            "measure_label": header_str,          # libellé EXACT
            "col_index": col_idx,                 # 👈 ordre Excel (1-based)
    ...
    # 4) dates (si on en a trouvé au moins une dans le mois)
